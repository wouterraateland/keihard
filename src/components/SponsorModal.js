...
const SponsorModal = ({ t, values, update, updateModified, submit }) => (
  <Modal name="sponsor" small height="480px">
    <H1>
      <Trans i18nKey="title"><strong>Thanks</strong></Trans>
    </H1>
    {values.success
      ? (values.payment_method === 'Bankoverschrijving'
        ? <Text center>
            {t('transfer', { amount: values.amount })} <Kbd>NL48 RABO 0118 539 108</Kbd> {t`to`} <Kbd>Wouter Raateland</Kbd> {t`stating`} <Kbd>Sponsoring {values.name}</Kbd> {t`confirm`}.
          </Text>
        : <Text center>{t`success`} <span role="img" aria-label="party">🎉</span></Text>)
      : <Form>
          <InputGroup>
            <Input block label={t`name`} name="name" type="text" value={values.name} onChange={update('name')} required />
            <Input block label={t`email`} name="email" type="email" value={values.email} onChange={update('email')} required />
          </InputGroup>
          <InputGroup>
            <Input block label={t`amount`} type="text" value={`€${values.amount > 0 ? values.amount : ''}`} onChange={updateModified('amount')} />
            <Input block label={`${t`payment method`}: ${t(`methods.${values.payment_method}`)}`} component={PaymentMethods} />
          </InputGroup>
          <InputGroup>
            <Input
              type="radio"
