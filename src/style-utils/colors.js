...
export const getColorFormat = (color) => {
  ...
  const format = Object.keys(formats).find((format) =>
    formats[format].test(color)
  );
...
class Color {
  ...
  toRgba() {
    switch (this.format) {
      case "singleHex":
        this.values = [...this.values.map((c) => parseInt(c.repeat(2), 16)), 1];
        break;
      case "doubleHex":
        this.values = [...this.values.map((c) => parseInt(c, 16)), 1];
        break;
      case "rgb":
      ...
      case "rgba":
        break;
      case "hsl":
        this.values = [...hslToRgb(...this.values), 1];
        break;
      case "hsla":
        this.values = [...hslToRgb(...this.values), this.values[3]];
        break;
      default:
        break;
    }
  ...
  toHsla() {
    switch (this.format) {
      ...
      case "doubleHex":
        this.toRgba();
        this.values = [...rgbToHsl(...this.values), this.values[3]];
        break;
      case "rgb":
        this.values = [...rgbToHsl(...this.values), 1];
        break;
      case "rgba":
        this.values = [...rgbToHsl(...this.values), this.values[3]];
        break;
  ...
  setAlpha(alpha) {
    if (this.format !== "rgba" && this.format !== "hsla") {
      this.format === "hsl" ? this.toHsla() : this.toRgba();
    }
...
export const opacity = (color, alpha = 1) =>
  new Color(color).setAlpha(alpha).toColorString();
...
export const lighten = (color, amount = 0) =>
  modifyHsla(0, 0, amount, 0)(color);
export const darken = (color, amount = 0) =>
  modifyHsla(0, 0, -amount, 0)(color);
